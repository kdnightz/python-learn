...
4. 变量赋值不改变 原本左边指向的右边 的值，而是将左边指向新的内存对象。

5. 三引号字符串 支持跨行

6. 字符串里有引号：用三引号

7. 字符串元素的索引从0开始，负数反过来表示索引

8. 字符串 切片的位置！（从左向右，从**0**开始。从右向左，最右**-1**。以左方为索引位置）左闭右开

9. py的函数定义， 注意缩进。

10. 函数必须先定义，再调用，上下关系。

11. 函数的返回，`return ‘’`。引号里代表这个函数的返回，获取后输出。

12. 在定义函数时，赋缺省值，即default

13. 列表里边可存函数，可存列表 `nameList = [1,2,3，['hello'], [7,8,9] ]`

14. 列表的sequence操作，即索引切片

15. 列表特性-存储内容的改变。直接`a[i]=xxx,a[i][j]=xxx`

16. 元组（Tuple）：和列表不同的是，它的内容是无法改变的。`nameList=（）`，圆括号用来表示一个元组。`a = （1,2,3.14，‘hello’，[7,8,9]）`如果元组中只有一个元素，也要加逗号，即`a = (1,)`,否则对于`a = （1）`会被视为是数字1.

17. (**sequence** 下)判断 元素 是否存在于列表或元组里 用 `in`，返回`false`或`true`

18. 多个变量同时赋值：`x,y = (1,2)`  ,x 的值为1，y 的值为2

19. 列表同时赋值：`name,age = ['李逵',33]` ,name 的值为 '李逵'，age的值为 33

20. 不对的例子：`x,y = (1,2,3)    a,b = [1]`

21. 且 的条件组合：**and**

22. 或 的条件组合：**or**

23. 如果 and  和 or  一起使用，则**先计算 and**，其结果在和 or 一起计算。

24. **not**  翻转表达式。

25. 如果 and  or not 一起计算，则 先计算 **not**，再计算 **and**，最后是 **or**。比如：`not 4 > 6 or 'sk' == 'ok' and 4 > 3`.则先计算 **not 4 > 6** ，得到 **true**，再计算 **'sk' == 'ok' and 4 > 3**，得到 **false**。最后计算 **or**， 就是 **true or false** 。最后结果是 **true**。
